...
dateClicked:function(){
	...
	for(var e=0;e<this.activeDates.length;e++)
		this.activeDates[e].addEventListener("click",function(e){
			...
			previousSelection = this,this.setAttribute("data-calendar-selected","true");
			if (daySelection == 0){daySelection = 1; t.startDaySelection();}else{daySelection = 0; t.endDaySelection();}
			setTimeout(t.removeActiveClass(),1000);
		})
},
...
startDaySelection:function(){
	var selectedDate = document.querySelectorAll('[data-calendar-selected="true"]');
	var initialColor = selectedDate[0].style.backgroundColor;
	selectedDate[0].style.backgroundColor = "red";
	startDate = selectedDate[0].getAttributeNode("data-calendar-date").value;
	selectedDate[0].removeAttribute("data-calendar-selected");
	//this.capCalculating();
	setTimeout(function(){selectedDate[0].style.backgroundColor = initialColor},1000);
},
endDaySelection:function(){
	var selectedDate = document.querySelectorAll('[data-calendar-selected="true"]');
	var initialColor = selectedDate[0].style.backgroundColor;
	selectedDate[0].style.backgroundColor = "red";
	endDate = selectedDate[0].getAttributeNode("data-calendar-date").value;
...
capCalculating:function(){
	if (startDate != undefined && endDate != undefined){
		...
		var cap = Math.round(Math.abs((selectedStartDate.getTime() - selectedEndDate.getTime())/86400000) + 1);
		...
		document.getElementById("sick_days_0").innerHTML = (cap-offDays);
	}
},
};